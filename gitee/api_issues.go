...
/*
IssuesApiService 获取issue关联的Pull Requests
 * @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param owner 仓库所属空间地址(企业、组织或个人的地址path)
 * @param repo 仓库路径(path)
 * @param number Issue 编号(区分大小写，无需添加 # 号)
...
type GetV5ReposOwnerIssuesNumberPullRequestsOpts struct {
	AccessToken optional.String
}

func (a *IssuesApiService) GetV5ReposOwnerIssuesNumberPullRequests(ctx context.Context, owner string, repo string, number string, localVarOptionals *GetV5ReposOwnerIssuesNumberPullRequestsOpts) ([]PullRequest, *http.Response, error) {
	var (
		localVarHttpMethod  = strings.ToUpper("Get")
		localVarPostBody    interface{}
		localVarFileName    string
	...
	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/v5/repos/{owner}/issues/{number}/pull_requests"
	localVarPath = strings.Replace(localVarPath, "{"+"owner"+"}", fmt.Sprintf("%v", owner), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"number"+"}", fmt.Sprintf("%v", number), -1)
	...
	localVarQueryParams.Add("repo", repo)
